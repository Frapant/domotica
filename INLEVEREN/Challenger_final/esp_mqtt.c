...
bool esp_mqtt_subscribe(const char *topic, int qos, uint32_t timeout_ms)
{
    if (!topic) return ESP_ERR_BAD_PARAM;

    DEBUG_PRINT("SUBSCRIBING TO %s",topic);
    ...
    return stuur_cmd_ok( "OK", cmd, timeout_ms);
}
...
bool mqtt_pubraw(const char *topic, const char *payload)
{
    ...
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "AT+MQTTPUBRAW=0,\"%s\",%d,1,0\r\n", topic, len);
    DEBUG_PRINT("MQTT publish: AT+MQTTPUBRAW");
    ...
    DEBUG_PRINT("PUBRAW verzonden (controleer +MQTTPUB:OK/FAIL in output)");
    return true;
}
